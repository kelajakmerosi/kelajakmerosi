...
interface GeneralSectionPageProps {
  sectionId: string
  subjectId: string
  onBack: () => void
...
export function GeneralSectionPage({ sectionId, subjectId, onBack, onTopicSelect }: GeneralSectionPageProps) {
  ...
  const generalSection = useMemo(() => {
    ...
    return subject.sections.find(s => s.type === 'general' && s.id === sectionId) ?? null
  }, [subject, sectionId])

  const sectionTopics = useMemo(() => {
    if (!subject || !generalSection) return []
    return generalSection.topicIds
      .map(id => subject.topics.find(t => t.id === id))
      .filter(Boolean) as typeof subject.topics
  }, [subject, generalSection])
  ...
  const sectionTitle = generalSection?.title || 'Umumiy'
  ...
  return (
    <div className="page-content fade-in">
      ...
      <div className={styles.header}>
        ...
        <div>
          <h2 className={styles.title} style={{ color: subject.color }}>{sectionTitle}</h2>
          <p className={styles.meta}>{subjectName} · {sectionTopics.length} {t('lessons')}</p>
        </div>
      </div>
            ...
            return (
              <div key={topic.id}>
                <TopicRow
                  ...
                  onAction={handleOpen}
                />
                {idx < sectionTopics.length - 1 && <Divider margin="0 20px" />}
              </div>
            )
          })}
        </GlassCard>
