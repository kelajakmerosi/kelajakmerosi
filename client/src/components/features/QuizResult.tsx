...
import { Smile, ThumbsUp, BookOpen, Trophy, ListChecks, CheckCircle2, XCircle } from 'lucide-react'
    ...
    <div className="fade-in">
      ...
      <GlassCard padding={40} className={styles.scoreCard}>
        ...
        <div className={styles.resultIcon}>
          {pct >= 80 ? <Smile size={48} color="var(--success)" /> 
           : pct >= 50 ? <ThumbsUp size={48} color="var(--accent)" /> 
           : <BookOpen size={48} color="var(--warning)" />}
        </div>
        ...
        {allOk
          ? <Alert variant="success" className={styles.alert}><Trophy size={18} /> {t('completed')}!</Alert>
              ...
              <div key={q.id} className={styles.reviewItem}>
                <div className={styles.reviewQ}>
                  ...
                  <span style={{ marginLeft: 8, display: 'inline-flex', verticalAlign: 'middle' }}>
                    {isRight ? <CheckCircle2 size={18} color="var(--success)" /> : <XCircle size={18} color="var(--danger)" />}
                  </span>
                </div>
