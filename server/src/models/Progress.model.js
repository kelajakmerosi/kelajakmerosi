...
const findOne = async (userId, subjectId, topicId) => {
  const { rows } = await pool.query(
    `SELECT *
     FROM user_lesson_progress
     WHERE user_id = $1 AND subject_id = $2 AND topic_id = $3
     LIMIT 1`,
...
const markActivityDay = async (userId, at = new Date()) => {
...
const listActivityDays = async (userId) => {
  const { rows } = await pool.query(
  ...
  return rows.map((row) => normalizeDateKey(row.activity_date)).filter(Boolean);
};

const getStreakDays = async (userId) => {
...
const recordQuizAttempt = async (userId, subjectId, topicId, payload) => {
  const {
    score,
    masteryScore,
    totalQuestions,
...
const upsert = async (userId, subjectId, topicId, patch = {}) => {
...
const buildProgressPayload = async (userId) => {
  ...
  const attemptHistoryEntries = attemptRows.map((row) => ({
  ...
  const lastActivityAt = rows
    .map((row) => (row.last_activity_at ? new Date(row.last_activity_at).getTime() : 0))
